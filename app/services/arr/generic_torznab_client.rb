module Arr
  class GenericTorznabClient
    ...
    def self.call(arr_app:, name:, jackett_base_url:, jackett_api_key:, jackett_id:, connection: nil, caps_client: Jackett::TorznabCaps)
      new(arr_app:, name:, jackett_base_url:, jackett_api_key:, jackett_id:, connection:, caps_client:).call
    end

    def initialize(arr_app:, name:, jackett_base_url:, jackett_api_key:, jackett_id:, connection: nil, caps_client: Jackett::TorznabCaps)
      @arr_app = arr_app
      @name = name
    ...
    def call
      ...
      if (existing_indexer = find_existing_indexer)
        return success(existing_indexer.fetch("id"), nil, "Generic Torznab indexer already exists.")
      end
    ...
    private
      ...
      attr_reader :arr_app, :name, :jackett_base_url, :jackett_api_key, :jackett_id, :connection, :caps_client
      ...
      def find_existing_indexer
        response = http.get(INDEXER_PATH)
        return unless response.success?

        JSON.parse(response.body).find { |indexer| indexer["name"] == name }
      end

      def safe_find_existing_indexer
        find_existing_indexer
